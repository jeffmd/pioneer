...
RefCountedPtr<Graphics::Material> BaseLoader::ConvertMaterialDefinition(const MaterialDefinition &mdef, const Graphics::VertexFormatDesc &vtxFormat)
{
	...
	matDesc.pbrMap = !mdef.tex_pbr.empty();
	matDesc.enviroMap = !mdef.tex_enviro.empty();
	...
	if (!mdef.tex_pbr.empty())
		// use spec slot for pbr texture
		texture1 = Graphics::TextureBuilder::Model(mdef.tex_pbr).GetOrCreateTexture(m_renderer, "model");
